...
function toolStepHtml(steps,ctx){
  ctx=ctx||{};
  const subject=ctx.name||'questo software';
  const scenario=ctx.scenario||'un caso reale';
  const input=ctx.input||'un caso di prova';
  const result=ctx.result||'il risultato previsto';
  return steps.map(function(s,i){
    ...
    const success=s[3]||'';
    const lead=[
      'Partiamo dalla situazione concreta che vuoi sistemare. Non serve esplorare tutto '+subject+': in questo passaggio dobbiamo preparare soltanto ciò che serve al caso di prova.',
      'Adesso portiamo il caso dentro il sistema. È qui che un esempio diventa un dato che puoi seguire, modificare e controllare.',
      'A questo punto il processo prende forma. Prima di aggiungere funzioni, chiarisci quale informazione entra, quale decisione viene presa e quale azione deve avvenire dopo.',
      'Ora facciamo la prova completa. Testare un solo caso ci permette di riconoscere subito dati mancanti, duplicati o comportamenti inattesi.',
      'Con il test riuscito possiamo togliere dal lavoro manuale il passaggio che crea più attrito. La regola deve rimanere semplice e controllabile.',
      'Solo adesso ha senso replicare il modello. Prova un secondo caso leggermente diverso e verifica che il risultato resti coerente.'
    ][Math.min(i,5)];
    const example=[
      'Usa un solo caso rappresentativo: '+input.toLowerCase()+'. Deve essere abbastanza reale da rappresentare il lavoro, ma piccolo da poterlo controllare a mano.',
      'Per esempio, puoi lavorare su un caso chiamato “Mario Rossi — richiesta di prova”. Servirà per capire dove vanno i dati e quali campi sono davvero indispensabili.',
      'Descrivi il comportamento in una frase: “quando succede X, prendi questi dati e fai Y”. Se non riesci a dirlo così, il flusso è probabilmente ancora troppo complesso.',
      'Non usare dieci casi insieme. Esegui la prova con il singolo esempio e seguilo dall’input fino a '+result.toLowerCase()+'.',
      'Chiediti quale azione non vuoi più ricordarti di fare a mano: creare un’attività, inviare un messaggio, aggiornare un record o spostare un documento.',
      'Quando il primo caso funziona, ripetilo con un secondo caso diverso. Se il comportamento resta coerente, hai una procedura abbastanza solida per iniziare a replicarla.'
    ][Math.min(i,5)];
    const trouble='Se la voce indicata non compare, non cambiare obiettivo: cerca la funzione equivalente nell’area indicata, controlla account e permessi e ripeti la prova con una sola variabile alla volta.';
    return '<section class="article-step" id="passo-'+(i+1)+'">'+
      '<div class="step-head"><span class="step-no">'+String(i+1).padStart(2,'0')+'</span><div><div class="eyebrow">PASSO '+(i+1)+' · FACCIAMOLO INSIEME</div><h2>'+esc(title)+'</h2></div></div>'+
      '<p>'+esc(lead)+'</p><p class="article-paragraph">'+esc(intro)+'</p>'+
      '<div class="action"><b>Adesso fai questo.</b> '+esc(action)+'</div>'+
      '<p class="examplebox"><b>Esempio pratico.</b> '+esc(example)+'</p>'+
      '<div class="success"><b>Come capisci che è fatto.</b> '+esc(success)+'</div>'+
      '<details class="trouble"><summary>Se qualcosa non torna</summary><p>'+esc(trouble)+'</p></details>'+
    '</section>';
  }).join('');
}
...
function baseCss(){
  return [
    ':root{--bg:#050711;--paper:#0f1526;--paper2:#111a2d;--ink:#f7f9ff;--muted:#aab5c7;--line:rgba(255,255,255,.09);--brand:#7c5cff;--brand2:#5b8cff;--ok:#36d99d;--warn:#ffd166;--shadow:0 30px 100px rgba(0,0,0,.30)}',
    '*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:radial-gradient(circle at 12% 0%,rgba(124,92,255,.16),transparent 26%),radial-gradient(circle at 88% 0%,rgba(91,140,255,.10),transparent 24%),linear-gradient(180deg,var(--bg),#0a0f1c);color:var(--ink);font-family:Inter,system-ui,-apple-system,"Segoe UI",sans-serif;line-height:1.8}.wrap{width:min(1080px,calc(100% - 28px));margin:auto}',
    '.top{position:sticky;top:0;z-index:20;background:rgba(5,7,17,.86);backdrop-filter:blur(16px);border-bottom:1px solid var(--line)}.nav{height:66px;display:flex;align-items:center;justify-content:space-between}.logo{font-weight:950;letter-spacing:.17em}.logo b{color:var(--brand)}.nav a{color:#d5dcaa;text-decoration:none;border:1px solid var(--line);background:rgba(255,255,255,.03);padding:8px 11px;border-radius:999px;font-size:10px;font-weight:900}',
    '.hero{padding:64px 0 30px}.eyebrow{font-size:9px;font-weight:950;letter-spacing:.12em;text-transform:uppercase;color:#bdb4ff}.hero h1{max-width:900px;margin:13px 0 14px;font-size:clamp(40px,6.3vw,70px);line-height:.98;letter-spacing:-.06em}.grad{background:linear-gradient(100deg,#fff,#ad9dff 47%,#79aaff);-webkit-background-clip:text;background-clip:text;color:transparent}.hero p{max-width:850px;margin:0;color:#aab5c7;font-size:16px;line-height:1.85}.chips{display:flex;gap:7px;flex-wrap:wrap;margin-top:18px}.chip{padding:7px 10px;border:1px solid var(--line);border-radius:999px;background:rgba(255,255,255,.03);color:#aebacd;font-size:9px;font-weight:900}',
    '.page{padding:22px 0 80px}.grid{display:grid;grid-template-columns:minmax(0,1fr) 270px;gap:27px;align-items:start}.article{min-width:0}.card,.sidecard,.toc{background:linear-gradient(145deg,rgba(16,23,40,.96),rgba(8,12,22,.98));border:1px solid var(--line);border-radius:20px;box-shadow:var(--shadow)}.section{padding:30px}.section h2{margin:0 0 12px;font-size:30px;line-height:1.15;letter-spacing:-.035em}.section p{margin:0 0 14px;color:#aab5c7;font-size:15px;line-height:1.9}',
    '.toc{padding:22px 24px;margin-bottom:20px}.toc-title{font-weight:950;font-size:15px;margin-bottom:8px}.toc ol{margin:0;padding-left:20px}.toc li{margin:6px 0}.toc a{color:#c3cbd8;text-decoration:none;font-size:12px}.toc a:hover{color:#bdb4ff}',
    '.facts{display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin-top:18px}.fact{padding:13px;border:1px solid rgba(255,255,255,.07);border-radius:12px;background:rgba(255,255,255,.018)}.fact b{display:block;font-size:8px;letter-spacing:.08em;color:#b0b9c9}.fact span{display:block;margin-top:5px;color:#909db0;font-size:11px;line-height:1.6}',
    '.leadbox{padding:18px 20px;border-left:4px solid var(--brand);background:rgba(124,92,255,.055);border-radius:0 12px 12px 0;margin-top:18px}.leadbox b{display:block;font-size:13px;margin-bottom:5px}.leadbox span{color:#a7b2c4;font-size:13px;line-height:1.75}',
    '.article-step{padding:35px 0;border-top:1px solid var(--line);scroll-margin-top:82px}.step-head{display:flex;gap:14px;align-items:flex-start}.step-no{width:42px;height:42px;display:grid;place-items:center;border-radius:12px;background:rgba(124,92,255,.10);color:#c9c1ff;font-weight:950;flex:0 0 auto}.article-step h2{margin:2px 0 10px;font-size:27px;line-height:1.2;letter-spacing:-.03em}.article-step>p{margin:0;color:#aab5c7;font-size:15px;line-height:1.9}.article-paragraph{margin:13px 0 0;color:#aab5c7;font-size:14.5px;line-height:1.9}.action{margin:18px 0 0;padding:15px 17px;border-left:3px solid var(--brand);background:rgba(124,92,255,.055);color:#cbd4e1;font-size:13.5px;line-height:1.8}.action b{color:#c9c1ff}.examplebox{margin:18px 0 0;color:#9eabbe;font-size:13px;line-height:1.8}.examplebox b{color:#ffd98a}.success{margin:17px 0 0;padding:13px 15px;border:1px solid rgba(54,217,157,.17);background:rgba(54,217,157,.035);border-radius:12px;color:#a8d9c4;font-size:12.5px;line-height:1.75}.success b{color:#75e5b7}.trouble{margin-top:15px;border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:11px 13px;background:rgba(255,255,255,.018)}.trouble summary{cursor:pointer;font-size:12px;font-weight:850;color:#d6dde8}.trouble p{margin:8px 0 0!important;color:#9ca9bb!important;font-size:12px!important;line-height:1.7!important}',
    '.side{position:sticky;top:80px}.sidecard{padding:18px}.sidecard+.sidecard{margin-top:12px}.sidecard h3{margin:0 0 10px;font-size:13px}.sidecard a{display:block;padding:8px 0;border-bottom:1px solid rgba(255,255,255,.06);color:#b6c0cf;text-decoration:none;font-size:10px;line-height:1.5}.sidecard p{margin:0;color:#8996aa;font-size:10px;line-height:1.7}.side .pill{display:inline-flex;margin-top:9px;padding:6px 8px;background:rgba(124,92,255,.10);color:#c2b9ff;border-radius:999px;font-size:8px;font-weight:900}',
    '.checklist{padding:22px;background:linear-gradient(145deg,rgba(16,23,40,.96),rgba(8,12,22,.98));border:1px solid var(--line);border-radius:18px;margin-top:20px}.checklist h2{margin:0 0 9px;font-size:21px}.checklist li{margin:8px 0;color:#9eabbe;font-size:12px;line-height:1.7}',
    '.cta{padding:25px;margin-top:20px;border-radius:18px;background:linear-gradient(135deg,#6357e8,#4f8df7);color:#fff}.cta h2{margin:0 0 7px;font-size:23px}.cta p{margin:0 0 13px;color:rgba(255,255,255,.88);font-size:13px;line-height:1.7}.btn{display:inline-flex;padding:10px 14px;border-radius:10px;background:#fff;color:#4338a9;text-decoration:none;font-size:10px;font-weight:950}',
    '.related{padding:22px;margin-top:20px}.related h2{margin:0 0 7px;font-size:20px}.related a{display:flex;justify-content:space-between;gap:10px;padding:10px 0;border-bottom:1px solid rgba(255,255,255,.06);color:#c1c9d5;text-decoration:none;font-size:11px}.related span{color:#7d8a9e;font-size:9px}',
    '.visual{margin-top:20px;padding:18px;border-radius:16px;background:#060a12;color:#fff;border:1px solid rgba(255,255,255,.08)}.visual-head{display:flex;justify-content:space-between;gap:10px;align-items:center}.visual-title{font-weight:950}.visual-label{font-size:9px;letter-spacing:.08em;color:#7d8aa0}.visual-grid{display:grid;grid-template-columns:70px 1fr;gap:12px;margin-top:13px}.visual-nav div{height:12px;margin-bottom:6px;border-radius:5px;background:rgba(255,255,255,.07)}.visual-nav .active{height:18px;background:rgba(124,92,255,.28);border:1px solid rgba(124,92,255,.5)}.visual-main{padding:13px;border:1px solid rgba(255,255,255,.08);border-radius:11px;background:rgba(255,255,255,.025)}.visual-main h4{margin:0 0 8px;font-size:13px}.visual-field{padding:9px;border-radius:8px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);margin-top:7px}.visual-field b{display:block;font-size:7px;color:#8e9bb3;letter-spacing:.08em}.visual-field span{display:block;margin-top:3px;font-size:10px;color:#e8edf7}.visual-note{margin-top:9px;font-size:9px;color:#8794a9;line-height:1.5}',
    'footer{padding:32px 0 55px;border-top:1px solid var(--line);text-align:center;color:#68768d;font-size:9px}',
    '@media(max-width:860px){.grid{grid-template-columns:1fr}.side{position:static}.facts{grid-template-columns:1fr 1fr}}@media(max-width:560px){.wrap{width:calc(100% - 20px)}.hero{padding-top:40px}.hero h1{font-size:39px}.hero p{font-size:14px}.section{padding:19px}.facts{grid-template-columns:1fr}.article-step h2{font-size:22px}}'
  ].join('');
}
...
function toolPage(tool,tut,d,all){
  ...
  return '<!doctype html><html lang="it"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#ffffff"><meta name="description" content="'+esc('Guida completa a '+name+': spiegazione, preparazione, procedura passo passo, esempi, controllo finale ed errori comuni.')+'"><link rel="canonical" href="'+url+'"><title>'+esc('Come usare '+name+' passo passo | PROJECT-X')+'</title><style>'+baseCss()+'</style></head><body>'+
    ...
    toolStepHtml(steps,{name:name,scenario:d&&d.scenario,input:d&&d.input,result:d&&d.result})+visual(tool,d)+'</section>'+
...
function goalPage(g,goals){
  ...
  return '<!doctype html><html lang="it"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="'+esc(g.mission+' '+g.focus)+'"><link rel="canonical" href="'+url+'"><title>'+esc(g.title)+' | PROJECT-X</title><style>'+baseCss()+'</style></head><body>'+
    ...
    '<section id="procedura" class="card section" style="margin-top:20px"><div class="eyebrow">PROCEDURA</div><h2>Passo dopo passo</h2><p>'+esc(g.setup||'Segui i passaggi in ordine e verifica ogni risultato prima di proseguire.')+'</p>'+toolStepHtml(steps,{name:name,scenario:d&&d.scenario,input:d&&d.input,result:d&&d.result})+'</section>'+
