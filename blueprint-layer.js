...
function buildNodes(answers,result){
 var E=window.ProjectXEngine||{}, p=primary(result), profile=result&&result.profile||{}, entries=Object.entries(profile).sort(function(a,b){return Number(b[1])-Number(a[1])}).slice(0,3);
 var nodes=[{n:"01",t:"Problema",d:answers&&answers.painPoint||"Bisogno operativo da strutturare"},{n:"02",t:"Nucleo",d:p&&p.name||"Software principale da definire"},{n:"03",t:"Automazione",d:labelFor(E,entries[0]&&entries[0][0]||"automation")},{n:"04",t:"Controllo",d:labelFor(E,entries[1]&&entries[1][0]||"documents")},{n:"05",t:"Misurazione",d:labelFor(E,entries[2]&&entries[2][0]||"projects")}];
 return nodes;
}
function mount(){
 if(document.getElementById("projectxBlueprintLive")) return;
 var result=window.__PROJECTX_RESULT__||window.projectXResult||null;
 var answers=window.__PROJECTX_ANSWERS__||{};
 if(!result) return;
 var nodes=buildNodes(answers,result), wrap=document.createElement("section");wrap.id="projectxBlueprintLive";
